...
class BooleanNode(object):
    ...
    def __init__(self, node_id, neighbors:list, bf : BooleanFunction, init_state=r_bool):
        self.__node_id = node_id
        self.__neighbors = neighbors
        self.boolfun = bf
        self.__hash = hash(str(self.__node_id))
    ...
    def to_json(self) -> dict:
        ...
        """
        return {'id':self.nid(), 'inputs': self.neighbors(), 'bf':self.boolfun.to_json()}
...
def bnstate_distance(s1:dict, s2:dict, crit = lambda ds: ds.count(False)):
    '''
